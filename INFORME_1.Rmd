---
title: "ESPACIAL"
author: "María Paula Camargo Rincón; Laura Katherine Castiblanco Martínez;"
date: "2026-02-08"
output: pdf_document
---
# Librerías
```{r, show = False}
#---------------------------------- Paquetes ----------------------------------#
#install.packages("ppcor")
#--------------------------------- Librerías ----------------------------------#
library(readr) #leer csv
library(dplyr) #manejo de datos
library(ggplot2) #gráficas
library(knitr) #Para mostrar tablas
library(ppcor) #correlación parcial
```

# Data

La base de datos cuenta con el nombre de los condados y distintos identificadores como el código FIPS del condado, con variables geográficas como el área y perímetro, y con variables sobre la salud, como el número total de casos (L) o la población en riesgo (POP), desagregadas por género (Masculino, Femenino), raza (Blanco, Negro) y año (1968, 1978 y 1988). Teniendo en cuenta la descripción anterior la varible "LMW68" sería el número de casos de cáncer de pulmón de hombres blancos en el año 1968 y la variable "POPFB88", sería la población en riesgo de mujeres negras del año 1988. Además de esto se cuenta con vairables como "LM68" o "POPF88" lo que sería el total por género, sin embargo esto más adelante será retirado debido a su alta correlación y redundancia de información. 
```{r}
#------------------------------------ Data ------------------------------------#
ohlung <- read_csv("ohiolung/ohlung.csv")
head(ohlung,10)
```

## Limpieza dela base de datos
```{r}
ohlung %>% select(-c("LM68","LF68","LM78","LF78","LM88","LF88",
                     "POPM68","POPF68","POPM78","POPF78","POPM88","POPF88"))
tabla1 <- ohlung %>% select(COUNTYID,NAME,AREA,PERIMETER,Casos=LFW88,
                            Poblacion=POPFW88)
tabla2 <- ohlung %>% select(COUNTYID,NAME,AREA,PERIMETER,Casos=LFB88,
                            Poblacion=POPFB88)
mujeres88 <- bind_rows(tabla1,tabla2)
mujeres88 <- mujeres88 %>% mutate(Raza=c(rep("W",88),rep("B",88))) 
mujeres88$Raza <- as.factor(mujeres88$Raza)
str(mujeres88)
```

```{r}
boxplot(Casos ~ Raza, data=mujeres88)
```
Los casos son menos dispersos en los negros que en los blancos.
```{r}
plot(mujeres88$Casos)
```

```{r}
casos_condado <- mujeres88 %>%
  group_by(NAME) %>%
  summarise(casos = sum(Casos, na.rm = TRUE)) %>% arrange(desc(casos))

ggplot(head(casos_condado,15), aes(x = reorder(NAME, casos), y = casos)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(
    title = "Casos de cáncer en mujeres (1988)",
    x = "Condado",
    y = "Número de casos"
  ) +
  theme_minimal()

```

```{r}
top_condados <- mujeres88 %>%
  group_by(NAME) %>%
  summarise(total_casos = sum(Casos, na.rm = TRUE)) %>%
  slice_max(total_casos, n = 10)


ggplot(
  mujeres88 %>% filter(NAME %in% top_condados$NAME),
  aes(
    x = reorder(NAME, ave(Casos, NAME, FUN = sum)),
    y = Casos,
    fill = Raza
  )
) +
  geom_col() +
  coord_flip() +
  labs(
    title = "Casos de cáncer en mujeres por condado y raza (1988)",
    x = "Condado",
    y = "Número de casos",
    fill = "Raza"
  ) +
  theme_minimal()


```

```{r}
ggplot(mujeres88 %>% filter(NAME %in% top_condados$NAME), 
       aes(x = NAME, y = Casos, fill = Raza)) +
  geom_col(position = "dodge") +
  coord_flip() +
  labs(
    title = "Casos de cáncer en mujeres por condado y raza (1988)",
    x = "Condado",
    y = "Número de casos",
    fill = "Raza"
  ) +
  theme_minimal()

```

```{r}
top_condados2 <-  mujeres88 %>%  
  group_by(NAME) %>%
  summarise(total_casos = sum(Casos, na.rm = TRUE),
            area = first(AREA)) %>% 
  slice_max(total_casos, n = 10)
kable(top_condados2, caption = "Top 10 condados con más casos")
```

```{r}
numericas <- mujeres88 %>% select(-c(COUNTYID,NAME,Raza))
cor(numericas)
pcor(numericas)
```
ALta correlación entre casos y población, se presenta correlación moderadamente alta entre el área y el perímetro. En las correlaciones parciales podemos ver que la relación entre las variables independientes, área y perímetro, y la variable dependiente casos, tiene una correlación baja, sin embargo llama la atención el hecho de que con área maneja una correlación lineal inversa, mientras que con perímetro maneja una correlación lineal directa. 

```{r}
lm <- lm(Casos~Raza+PERIMETER+Poblacion,data = mujeres88)
summary(lm)
```

```{r}

```